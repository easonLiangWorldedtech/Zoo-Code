...
- **更安全的檔案存取** — 使用獨立的讀取與寫入允許清單，控制 Zoo Code 可以存取的路徑。
- **更多模型選擇** — GLM 5.3 現已透過 Z.ai 提供，Gemini provider 也新增了 Gemini 3.5 Flash Lite 和 Gemini 3.1 Flash Lite。
- **更可靠的日常工作流程** — 改善強化了任務復原與歷史記錄、provider 串流與工具使用、設定匯入，以及不同 IDE 主題下的可讀性。
