...
describe("AnthropicHandler adaptive effort envelope (DTE series 2/5)", () => {
	...
	describe("envelope omission (out-of-range or non-adaptive)", () => {
		...
		it("omits output_config on the default requestParams branch for an out-of-range effort", async () => {
			// Unknown model id -> default (non-cache-control) requestParams branch; the
			// guessed model info (claude-opus-4-7 substring) is adaptive-capable, but the
			// settings effort is out-of-range, so the envelope is omitted on that branch too.
			const handler = makeHandler({
				apiModelId: "claude-opus-4-7-custom",
				enableReasoningEffort: true,
