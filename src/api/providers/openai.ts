...
export class OpenAiHandler extends BaseProvider implements SingleCompletionHandler {
	...
	override async *createMessage(
		...
		if (this.options.openAiStreamingEnabled ?? true) {
			...
			let stream
			try {
				const createOpts: OpenAI.RequestOptions = metadata?.abortSignal
					? isAzureAiInference
					...
					: isAzureAiInference
						...
						: {}
				stream = await this.client.chat.completions.create(requestOptions, createOpts)
			} catch (error) {
			...
			try {
				const createOpts: OpenAI.RequestOptions = metadata?.abortSignal
					...
					: this._isAzureAiInference(modelUrl)
						? { path: OPENAI_AZURE_AI_INFERENCE_PATH }
						: {}
				response = await this.client.chat.completions.create(requestOptions, createOpts)
			} catch (error) {
				throw handleOpenAIError(error, this.providerName)
			}
