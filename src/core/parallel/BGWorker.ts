...
/** Default heartbeat interval: 30 seconds between worker heartbeats */
const DEFAULT_HEARTBEAT_INTERVAL_MS = 30000

/** System prompt for background workers */
export function buildSystemPrompt(config: BGWorkerConfig, skillInstructions: string): string {
...
export function enrichWithContext(sharedContext: SharedWorkerContext): string | null {
...
function calculateCost(
    ...
    const protocol = getApiProtocol(
        apiProvider && !isRetiredProvider(apiProvider) ? (apiProvider as any) : undefined,
        modelId,
    )

    if (protocol === "anthropic") {
        return calculateApiCostAnthropic(
            { contextWindow: 200_000, supportsPromptCache: true, inputPrice: 15, outputPrice: 75, cacheWritesPrice: 1.875, cacheReadsPrice: 0.375 },
            inputTokens,
            outputTokens,
            cacheWriteTokens,
            cacheReadTokens,
...
export class BGWorker extends EventEmitter<BGWorkerEvents> {
    ...
    /** Heartbeat interval timer — emits periodic progress updates to BGWorkerManager */
    private heartbeatInterval?: NodeJS.Timeout

    /** Current action/tool being executed (for heartbeat reporting) */
    ...
    async start(): Promise<void> {
        ...
        const skillLoader = new BGWorkerSkillLoader(
            (this.provider as any).skillsManager,
        )
        ...
        // Start heartbeat interval — emit progress updates every N seconds
    ...
    private startHeartbeatTimer(): void {
        const intervalMs = (this.provider as any)?.stateCache?.["parallelTaskHeartbeatInterval"] ?? DEFAULT_HEARTBEAT_INTERVAL_MS
        this.heartbeatInterval = setInterval(() => this.emitHeartbeat(), Math.max(10000, intervalMs)) // Clamp to min 10s
    }
    ...
    /** Emit a heartbeat update to BGWorkerManager via stateUpdate event */
    private emitHeartbeat(): void {
        if (this._state === BgWorkerStateEnum.Queued || this._state === BgWorkerStateEnum.Failed) return

        const elapsedMs = Date.now() - this.startTime
        const maxToolCalls = this.config.maxToolCallsPerTask ?? DEFAULT_MAX_TOOL_CALLS_PER_TASK
        ...
        const heartbeat: WorkerHeartbeat = {
            ...
            maxToolCalls,
            timestamp: Date.now(),
            workflowId: this.config.workflowId, // TaskFlowAgent (Phase 7d): link heartbeat to parent workflow
        }

        // Emit as a stateUpdate — BGWorkerManager will aggregate and forward to webview
        ...
        // Also emit a dedicated heartbeat event for BGWorkerManager to pick up
        ...
        while (this._state === BgWorkerStateEnum.Running && !this.isProcessing) {
            ...
            try {
                ...
                if (this.totalCost > (this.config.maxCostPerTask ?? DEFAULT_COST_LIMIT)) {
                    console.log(`[BGWorker] Cost limit reached ($${this.totalCost.toFixed(2)} / $${(this.config.maxCostPerTask ?? DEFAULT_COST_LIMIT).toFixed(2)})`)
                    await this.attemptCompletion("max_cost_reached", `Cost limit reached: $${this.totalCost.toFixed(2)} of $${(this.config.maxCostPerTask ?? DEFAULT_COST_LIMIT).toFixed(2)}`)
                    return
                }
                ...
                if (totalTokens > (this.config.maxTokensPerTask ?? DEFAULT_TOKEN_LIMIT)) {
                    console.log(`[BGWorker] Token limit reached (${totalTokens} / ${(this.config.maxTokensPerTask ?? DEFAULT_TOKEN_LIMIT)})`)
                    await this.attemptCompletion("max_tokens_reached", `Token limit reached: ${totalTokens} of ${(this.config.maxTokensPerTask ?? DEFAULT_TOKEN_LIMIT)}`)
                    return
                }
                ...
                if (this.totalToolCalls >= (this.config.maxToolCallsPerTask ?? DEFAULT_MAX_TOOL_CALLS_PER_TASK)) {
                    console.log(`[BGWorker] Tool call limit reached (${this.totalToolCalls} / ${(this.config.maxToolCallsPerTask ?? DEFAULT_MAX_TOOL_CALLS_PER_TASK)})`)
                    await this.attemptCompletion("max_tool_calls_reached", `Tool call limit reached: ${this.totalToolCalls} of ${(this.config.maxToolCallsPerTask ?? DEFAULT_MAX_TOOL_CALLS_PER_TASK)}`)
                    return
                }
                ...
                if (this.totalToolCalls >= MAX_TOOL_CALLS_BEFORE_FORCED_COMPLETE) {
                    ...
                    await this.attemptCompletion("max_tool_calls_forced", `Forced completion after ${this.totalToolCalls} tool calls`)
                    return
                }
                ...
                const retentionLevel = this.config.contextRetention ?? DEFAULT_CONTEXT_RETENTION_PER_TYPE[this.config.taskType ?? Ptt.General]
                ...
                // Collect assistant messages and tool uses from chunks
                ...
                if (requestCost !== undefined) {
                    ...
                    this.totalCost += calculateCost(inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens, apiProvider, modelId)
                }
                ...
                if (toolUses.length > 0) {
                    ...
                    // Check for errors in tool results before adding to history
                    const hasErrors = results.some((r: any) => r.is_error)
                    if (hasErrors) {
                        console.log(`[BGWorker] Tool execution produced ${results.filter((r: any) => r.is_error).length} error(s)`)
                    }
                    ...
                    if (consecutiveEmptyCount >= MIN_ITERATIONS_BEFORE_IDLE && iteration >= MIN_ITERATIONS_BEFORE_IDLE) {
                        console.log(`[BGWorker] Idle detected after ${iteration} iterations (${consecutiveEmptyCount} consecutive empty)`);
                        await this.attemptCompletion("idle", "No tool calls for several iterations — task appears complete")
                        return
                    }
                }
    ...
    /**
     * Execute a list of tool calls (native + MCP).
     */
    private async executeTools(toolUses: Array<ToolUse | McpToolUse>): Promise<any[]> {
        const results: any[] = []
        ...
        for (const toolUse of toolUses) {
            ...
            // Track current action for heartbeat reporting
            this.currentAction = toolUse.name as string

            try {
                const result = await this.executeSingleTool(toolUse as ToolUse)
                results.push(result)
                this.totalToolCalls++

                // Record file change if applicable (for write tools)
                this.recordFileChangeIfApplicable(toolUse as ToolUse, result)
                ...
                // Clear current action after tool completes
                this.currentAction = ""
            }
        }
    ...
    private async executeSingleTool(toolUse: ToolUse): Promise<any> {
        ...
        const autoApprove = this.config.autoApprove ?? DEFAULT_AUTO_APPROVE_PER_TYPE[this.config.taskType ?? Ptt.General]
        ...
        switch (toolName) {
            ...
            case "write_to_file": {
                ...
                try {
                    const fs = await import("fs/promises")
                    await fs.writeFile(path, typeof content === "string" ? content : JSON.stringify(content, null, 2), "utf-8")
                    recordFileChange(this.sharedContext, this.getId(), path)
                    return {
                        type: "tool_result",
                        tool_use_id: (toolUse as any).id ?? "",
                ...
                try {
                    ...
                    for (const line of diffLines) {
                        if (line.startsWith("-") && !line.startsWith("---")) {
                            ...
                            result = result.replace(new RegExp(removedLine.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "m"), "")
    ...
    private recordFileChangeIfApplicable(toolUse: ToolUse, result: any): void {
        ...
        if (filePath && result?.content && typeof result.content === "string") {
            ...
            const isSuccess = successContent.some((s) => result.content.toLowerCase().includes(s.toLowerCase())) || !result.is_error
            if (isSuccess) {
    ...
    private async attemptCompletion(reason: string, summary?: string): Promise<void> {
        ...
        // Stop heartbeat timer before completing
        ...
        const result: BGWorkerResult = {
            ...
            durationMs,
            notificationMode: this.config.notificationMode ?? DEFAULT_NOTIFICATION_MODE_PER_TYPE[this.config.taskType ?? Ptt.General],
        }
    ...
    cancel(reason: string): void {
        if (this._state === BgWorkerStateEnum.Completed || this._state === BgWorkerStateEnum.Failed || this._state === BgWorkerStateEnum.Cancelled) {
            return
        }

        // Stop heartbeat timer before cancelling
        this.stopHeartbeatTimer()
    ...
    pause(): void {
        ...
        // Stop heartbeat timer while paused — resume will restart it
        this.stopHeartbeatTimer()
    ...
    resume(): void {
        ...
        // Restart heartbeat timer on resume
        this.startHeartbeatTimer()
