...
describe("computeTaskDepth", () => {
	...
	it("resolves an ancestor reached after exactly MAX_DEPTH_WALK hops", () => {
		// Chain of length MAX_DEPTH_WALK + 1 (t0 root .. t32). The top task sits
		// exactly MAX_DEPTH_WALK hops below the root, so it must resolve to that
		// depth rather than being rejected as too long.
		const nodes: Record<string, { parentTaskId?: string }> = {}
		for (let i = 0; i <= MAX_DEPTH_WALK; i++) {
			nodes[`t${i}`] = { parentTaskId: i === 0 ? undefined : `t${i - 1}` }
		}
		const lookup = (id: string) => nodes[id]
		expect(computeTaskDepth(`t${MAX_DEPTH_WALK}`, undefined, lookup)).toBe(MAX_DEPTH_WALK)
	})
